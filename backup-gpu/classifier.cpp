...
void Classifier::InjectRandomSEU()
{
  ...
  boost::dynamic_bitset<> fp_weight_bin(float_size, *reinterpret_cast<unsigned long*>(&fp_weight));

  int bit_index = rand() % float_size;
  std::cout << "[I] BIT FLIP AT POSITION " << bit_index << std::endl;

  fp_weight_bin[bit_index] = !fp_weight_bin[bit_index];

  float new_value = 0;
  const auto val = fp_weight_bin.to_ulong();
  memcpy(&new_value, &val, sizeof(float));
  
  std::cout << "[I] STD " << new_value << " FL " << " was " << fp_weight << std::endl;
